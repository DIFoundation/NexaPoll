...
const generateVotes = (status: string) => {
  if (status === 'pending') return { forVotes: 0, againstVotes: 0, abstainVotes: 0 };
...
const generateProposalsForDAO = (daoId: string, count: number = 4): Proposal[] => {
  ...
  return statuses.map((status, index) => {
    ...
    const daoCategory = daoId === '1' ? 'defi' : 
                       daoId === '2' ? 'nft' : 
                       daoId === '3' ? 'gaming' : 
                       daoId === '4' ? 'social' : 
                       daoId === '5' ? 'governance' : 'other';
    
    const proposalData = generateProposalData(daoCategory, status, index + 1);
...
const generateProposalData = (category: string, status: string, index: number) => {
  const commonTitles = {
    treasury: [
  ...
  // Select a title
  let title: string;
  const commonTitleType = Math.random() > 0.7 ? Object.keys(commonTitles)[Math.floor(Math.random() * 3)] : null;
  ...
  const description = `This proposal ${status === 'pending' ? 'aims to' : 'aimed to'} ${title.toLowerCase()}.
  
${generateProposalDetails(category, title)}`;
  
  return { title, description };
};

const generateProposalDetails = (category: string, title: string): string => {
